module ManagerRefresh::SaveCollection
  module Saver
    class ConcurrentSafeBatch < ManagerRefresh::SaveCollection::Saver::Base
          ...
          batch.each do |record|
            ...
            if inventory_object.nil?
              ...
              if !inventory_collection.check_changed? || record.changed?
                hashes_for_update << hash
              end
            end
          end

          # Persist in batches
          if hashes_for_update >= 1000
      ...
      # def update_records111!(inventory_collection, record, hash, inventory_object)
      #   record.assign_attributes(hash.except(:id, :type))
      #
      ...
      def update_records!(inventory_collection, all_attribute_keys, hashes)
        ...
        update_query = %{
          UPDATE #{table_name}
            SET
              #{all_attribute_keys_array.map {|key| "(#{table_name}.#{key} = thevalues.#{key})" }.join("\n")}
          FROM (
            VALUES
              #{hashes.map { |hash| "(#{all_attribute_keys_array.map { |x| ActiveRecord::Base.connection.quote(hash[x]) }.join(", ")})" }.join("\n,")}
