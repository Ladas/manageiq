  ...
  module Saver
    class ConcurrentSafeBatch < ManagerRefresh::SaveCollection::Saver::Base
      ...
      def update_or_destroy_records!(records_batch_iterator, inventory_objects_index, attributes_index, all_attribute_keys)
        ...
        # get_connection.execute(association.select(*select_keys).order("id ASC").to_sql).each do |record|
        # get_connection.query(association.select(*select_keys).order("id ASC").to_sql).each do |record|
